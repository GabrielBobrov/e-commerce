...
sequenceDiagram
    ...
    participant DB as PostgreSQL (Orders & Events)
    ...
    Note over Client, DB: Fase 1: Criação e Persistência (Síncrono)
    Client->>Controller: POST /api/orders (JSON)
    Controller->>Service: createOrder(OrderDomain)
    Service->>DB: INSERT Order (PENDING) + OrderEvent (published=false)
    ...
    loop A cada 20s
        ...
        Scheduler->>SNS: Publica OrderCreatedEvent
        SNS-->>Scheduler: Ack
        Scheduler->>DB: UPDATE order_events SET published=true
    end
    
    Note over SNS, Listener: Fase 3: Consumo e Processamento (Async)
    ...
    alt Pedido Já Existe
        ...
        Listener->>DB: Salva Pedido (INSERT)
    end
```
...
2.  **Serviço (Core)**:
    ...
    *   Gera um `orderNumber` temporário.
    *   Cria um `OrderEvent` (Outbox) com `published=false` contendo o payload do pedido.
    *   Salva **Pedido** e **Evento** na mesma transação de banco de dados.
    *   **Log**: `[SERVICE] Pedido e Evento salvos no banco com sucesso.`
...
3.  **Publicação (Scheduler - Batch)**:
    ...
    *   Reconstrói o `OrderCreatedEvent` a partir do payload salvo.
    *   Publica no SNS via `IOrderEventPublisherPort`.
    *   Atualiza o evento no banco para `published=true`.
    *   **Log**: `[SCHEDULER] Evento publicado com sucesso.`
...
5.  **Consumo (Infrastructure - Listener)**:
    ...
    *   Se não existir, salva o pedido (simulando processamento em outro contexto).
    *   **Log**: `[LISTENER] Pedido salvo no banco com sucesso.` ou `[LISTENER] Pedido já existe...`
